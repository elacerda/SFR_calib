...
if __name__ == '__main__':
    ...
    for i, b in enumerate(bases):
        ...
        ax.set_ylabel(r'kSFR [M$_\odot$ yr$^{-1}$]', fontsize=fs)
        # f.tight_layout()
        f.savefig('logZ_kSFR_%s.%s' % (b.replace('.', '_'), outputImgSuffix), dpi=300)
        plt.close(f)
        ...
        ax.set_ylabel(r'kSFR [M$_\odot$ yr$^{-1}$]', fontsize=fs)
        # f.tight_layout()
        f.savefig('Z_kSFR_%s.%s' % (b.replace('.', '_'), outputImgSuffix), dpi=300)
        plt.close(f)
    ...
    # f.tight_layout()
    f.savefig('logZ_kSFR_bases.%s' % outputImgSuffix, dpi=300)
    plt.close(f)
    ...
    ax.legend(loc=2, fontsize=fs-2, borderpad=0, frameon=False)
    # f.tight_layout()
    f.savefig('Z_kSFR_bases.%s' % outputImgSuffix, dpi=300)
    plt.close(f)
